...
fn day01_1(filename: &str) -> Result<i64, io::Error> {
    ...
    for line in reader.lines() {
        ...
        if numbers.len() == 2 {
            if let Ok(first) = numbers[0].parse::<i64>() {
                first_numbers.push(first);
            }
            if let Ok(second) = numbers[1].parse::<i64>() {
    ...
    for (first_number, second_number) in first_numbers.iter().zip(second_numbers.iter()) {
...
fn day01_2(filename: &str) -> Result<i64, io::Error> {
    ...
    let mut second_counts: Vec<(i64, i64)> = second_numbers
        .into_iter()
    ...
    while first_iterator.peek().is_some() && second_iterator.peek().is_some() {
